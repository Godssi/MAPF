...
int Planner::safe_distance(std::map<Agent, std::vector<std::vector<int>>> solution, Agent agent_i, Agent agent_j)
{
	...
	int size = 0;
	std::vector<std::vector<int>> paths_i = solution[agent_i]; // agent_i에 대한 경로 ex) {{1,2},{5,4}} 
	std::vector<std::vector<int>> paths_j = solution[agent_j]; // agent_j에 대한 경로 ex) {{4,6},{3,8}} 
	std::vector< std::vector<std::vector<int>>, std::vector<std::vector<int>>> self = { paths_i, paths_j }; // paths_i랑 paths_j를 계속 바꿔줘야함 for문 써서 
...
// 제약조건: 시간에 따른 갈 수 없는 좌표들
	...
	std::pair<int, int> pivot; // 충돌이 발생한 시간에서 좌표
	...
	while (true) { //aaaaa
		...
		if (Planner::dist(conf, pivot) >= 2 * Planner::robot_radius) break;  // 충돌 상황
		conflict_end_time++;
	}
...
std::vector<std::vector<int>> Planner::calculate_path(Agent agent, Constraints constraints, std::map<int, std::set<std::pair<int, int>>> goal_times) // 공간이랑 시간에 대해 모든 에이전트들의 충돌이 없는 경로를 반환
{
...
static std::vector<std::vector<std::vector<int>>> reformate(std::vector<Agent> agents, std::map<Agent, std::vector<std::vector<int>>> solution) // pad로 늘리고 솔루션이 새로 만들어짐
{
