...
// 1. 클래스 내 멤버 함수에서 멤버 변수와 다른 멤버 함수에 대한 접근이 안된다. (비정적 멤버 참조는 특정 개체에 상대적이어야한다.)
// 2. plan의 다중처리 개념 및 result에 대한 정확한 의미 파악 필요
// 3. st_planner에 맞게 우리의 단일 알고리즘 수정 필요
// 4. deep copy에 대한 이해 필요


std::vector<std::vector<std::pair<int, int>>> plan(std::vector<std::pair<int, int>> starts, std::vector<std::pair<int, int>> goals, std::function<vector<Agent>(std::vector<std::pair<int, int>>, std::vector<std::pair<int, int>>)> assign = min_cost, int max_iter = 200, int low_level_max_iter = 100, int max_process = 10, bool debug = false)
// 경로들을 반환해주는 하뭇
{
...
{
	map<int, set<p>> a;
	return st_planner.plan(agent.start, agent.goal, constraints.setdefault(agent, a), goal_times, low_level_max_iter, debug); // 충돌없는 경로 반환
}
//reformat/////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 
// 각 agent들의 경로(2차원 벡터) 길이 똑같이 만들고 저장한 3차원 벡터 반환
static std::vector<std::vector<std::pair<int, int>>> reformat(std::vector<Agent> agents, std::map<Agent, std::vector<std::pair<int, int>>> solution) // pad로 늘리고 솔루션이 새로 만들어짐
{
	std::map<Agent, std::vector<std::pair<int, int>>> solution = Planner::pad(solution);
...
// 여기서 solution에서 agent와 함께 있는 path들의 집합이 vector인데 path들은 위치인데 이걸 굳이 vector<int>로 해야되나?? 이러면 오류가 생길 수도 있지 않나?
{
	...
	{
		std::vector<std::vector<int>> path = elem.second;
	...
	for (auto& elem : solution) {
		...
		std::vector<std::vector<int>> padded = path;
		int added_len = max_ - padded.size();
		std::vector<int> end_value = padded.back();
