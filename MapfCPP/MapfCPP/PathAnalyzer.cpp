...
void averageDistance2Obstacle(vec3PInt results, vector<DynamicObstacle> DO)
{
    ...
    for (int i = 0; i < resultIdx; i++)
    {
        ...
        for (vec2PInt::iterator iter1 = results[i].begin(); iter1 != results[i].end(); iter1++)
        {
            ...
            for (auto iter2 = iter1->begin(); iter2 != iter1->end(); iter2++)
            {
                ...
                {
                    if (i < obIter->DoB_path.size())
                        distance = min(distance, sqrt(pow(iter2->first - obIter->DoB_path[i].first, 2) + pow(iter2->second - obIter->DoB_path[i].second, 2)));
                }
                Dis.push_back(distance);
    ...
    cout << "total : " << accumulate(ADO.begin(), ADO.end(), 0.0) / ADO.size() << "\n";
}
    ...
    for (int i = 0; i < resultIdx; i++)
    {
        ...
        for (vec2PInt::iterator iter1 = results[i].begin(); iter1 != results[i].end(); iter1++)
        {
            ...
            for (auto iter2 = iter1->begin(); iter2 != iter1->end(); iter2++)
            {
                ...
                for (auto obIter = Ob.begin(); obIter != Ob.end(); obIter++)
                {
                        distance = distance = min(distance, sqrt(pow(iter2->first - obIter->first, 2) + pow(iter2->second - obIter->second, 2)));
                }
                Dis.push_back(distance);
